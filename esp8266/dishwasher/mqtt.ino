...
const byte turn_off_cmd = 0x00;
//   timer        = 0x20;
const byte start_cmd    = 0x30;
const byte pause_cmd    = 0x33;
const byte resume_cmd   = 0x31;
const byte finish_cmd   = 0x40;
const byte water_cmd    = 0x50;
const byte drain_cmd    = 0x60;
// sounds
// E1 - on
// E2 - off
// E3 - click / error
...
void set_mode(char* mode) {
  if (strlen(mode) == 0) {
    ...
    if (strcmp(mode, "auto") == 0) {
        ...
        debug.printf("Unknown mode value: %s (should be auto, intense, eco, delicate, quick, quick_delicate or rinse)\n", mode);
    }
}
...
void set_state(char* state) {
  ...
  if (strcmp(state, "run") == 0) {
    if (controller_buff[1] == pause_cmd) {
      ...
      debug.printf("Unknown state value: %s (should be run, pause or stop)\n", state);
  }
}
...
void set_switch(char* value) {
  ...
  if (strcmp(value, "on") == 0) {
      ...
      debug.printf("Unknown switch value: %s (should be on or off)\n", value);
  }
}
